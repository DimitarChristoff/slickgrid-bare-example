...
const companyNames = Array.from({length: 30}, Faker.company.companyName);
...
dv.setItems(
  makeArray(300, id => {
    ...
    const data = {
      ...
      counterparty: _.sample(companyNames),
...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    grid.onHeaderRowCellRendered.subscribe((e, {node, column}) => {
      if (['historic', 'health'].indexOf(column.id) === -1) {
        ReactDOM.render(
    ...
    grid.onClick.subscribe((e, {row, column}) => {
      ...
      if (grid.getColumns()[cell.cell].id == 'currency') {
        const {currency} = dv.getItem(row);
        window.FSBL.Clients.LauncherClient.showWindow(
          ...
          },
          {
            ...
            dockOnSpawn: true,
            name: `GBP${currency}`,
            url: `https://uk.tradingview.com/chart/?symbol=FX%3AGBP${currency}`
          }
        );
      }
    });
